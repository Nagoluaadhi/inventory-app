...
const loadInventory = async () => {
  ...
  setUsers(res.data);
  setAllUsers(res.data); // <-- for dropdown
};
...
await axios.post('http://localhost:3001/api/clients', {
 ...
 const deleteUser = async (id) => {
  ...
  try {
    ...
    loadUsers(); // or whatever your function is to reload the user list
  } catch (err) {
...
<table className="w-full text-sm border">
  <thead className="bg-gray-100">
    <tr>
      ...
      <th>Client Name</th>
      <th>Address</th>
      <th>Assigned Branch</th> {/* 👈 Add this */}
      <th>Action</th>
    </tr>
  </thead>
  <tbody>
    {clients.map(client => {
      ...
      return (
        <tr key={client.id}>
          ...
          <td>{assignedUser?.username || '—'}</td> {/* 👈 Show branch user */}
          <td>
            <button onClick={() => deleteClient(client.id)} className="bg-red-600 text-white px-2 py-1 text-xs rounded">
              Delete
            </button>
          </td>
        </tr>
